...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function bindSystemButtons() {
        ...
        if (!systemSection) {
            console.warn('system-section not found');
            return false;
        }
        
        // 使用事件委托绑定系统管理区域的所有点击事件
    ...
    setTimeout(() => {
        // 默认加载 Jenkins 视图数据
        if (typeof window.loadJenkinsJobsView === 'function') {
            window.loadJenkinsJobsView();
        }
        ...
        // 如果Pipeline标签页是激活的，加载Pipeline数据
        ...
        // 默认加载 Jenkins 视图（登录后默认显示）
        if (typeof window.loadJenkinsJobsView === 'function') {
            console.log('Loading default Jenkins view...');
            window.loadJenkinsJobsView();
        } else {
            console.warn('loadJenkinsJobsView function not available, will retry...');
            // 如果函数还未加载，延迟重试
            setTimeout(function() {
                if (typeof window.loadJenkinsJobsView === 'function') {
                    window.loadJenkinsJobsView();
