...
function initMockData() {
    // 模拟环境数据
    mockEnvs = [
        { id: 1, name: 'dev', k8sCluster: 'cluster-dev', k8sNamespace: 'dev', harborProject: 'dev', status: 1 },
        { id: 2, name: 'uat', k8sCluster: 'cluster-uat', k8sNamespace: 'uat', harborProject: 'uat', status: 1 },
        { id: 3, name: 'pre', k8sCluster: 'cluster-pre', k8sNamespace: 'pre', harborProject: 'pre', status: 1 },
        { id: 4, name: 'prod', k8sCluster: 'cluster-prod', k8sNamespace: 'prod', harborProject: 'prod', status: 1 }
    ];
    ...
    // 模拟Jenkins节点数据
    mockNodes = [
        { id: 1, name: 'jenkins-build-01', nodeType: 'build', label: 'build', status: '在线', jenkinsUrl: 'http://jenkins:8080' },
        { id: 2, name: 'jenkins-build-02', nodeType: 'build', label: 'build', status: '在线', jenkinsUrl: 'http://jenkins:8080' },
        { id: 3, name: 'jenkins-deploy-01', nodeType: 'deploy', label: 'deploy', status: '在线', jenkinsUrl: 'http://jenkins:8080' },
        { id: 4, name: 'jenkins-deploy-02', nodeType: 'deploy', label: 'deploy', status: '在线', jenkinsUrl: 'http://jenkins:8080' }
    ];
...
function showSection(sectionId, element) {
    ...
    // 加载对应区域的数据
    if (sectionId === 'jenkins' && typeof loadJenkinsJobsView === 'function') {
        loadJenkinsJobsView();
    }
    ...
    switch(sectionId) {
        ...
        case 'system':
            ...
            setTimeout(() => {
                const activePipelinePanel = document.getElementById('pipeline-config');
                if (activePipelinePanel && activePipelinePanel.classList.contains('active')) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        ...
        // 检查当前激活的标签页，如果是pipeline标签页，则加载pipeline数据
        const activePipelinePanel = document.getElementById('pipeline-config');
        if (activePipelinePanel && activePipelinePanel.classList.contains('active')) {
            if (typeof loadPipelines === 'function') {
                loadPipelines();
        ...
        // 默认显示任务列表
        const tasksMenuItem = document.querySelector('.menu-item[onclick*="tasks"]');
        if (tasksMenuItem) {
            tasksMenuItem.click();
        }
    }, 100);
