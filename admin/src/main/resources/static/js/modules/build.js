                    ...
                    return `
                        <div class="build-job-item">
                            ...
                            <div class="build-job-meta">
                                ...
                                <span>Jenkins节点: ${job.jenkinsNode || '-'}</span>
                                <span>创建时间: ${createTime}</span>
                                ${startTime !== '-' ? `<span>开始时间: ${startTime}</span>` : ''}
                                ${endTime !== '-' ? `<span>结束时间: ${endTime}</span>` : ''}
                                ${job.jenkinsBuildNumber ? `<span>构建号: #${job.jenkinsBuildNumber}</span>` : ''}
                            </div>
                            ${job.jenkinsJobUrl ? `
                                <div style="margin-top: 8px;">
                                    <a href="${job.jenkinsJobUrl}" target="_blank" style="color: #667eea; text-decoration: none; font-size: 13px;">
                                        查看构建日志 →
                                    </a>
                                </div>
                            ` : ''}
...
async function showAddBuildTaskModal() {
    ...
    if (pipelines.length === 0) {
        alert('暂无可用Pipeline模板，请先在系统设置的Pipeline管理中创建Pipeline模板');
        return;
    }
