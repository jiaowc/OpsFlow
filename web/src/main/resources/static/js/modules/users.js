...
async function loadUsers() {
    ...
    userList.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 40px; color: #999;">暂无用户数据</td></tr>';
}
...
async function showAddUserModal() {
    ...
    const content = `
        <form id="addUserForm">
            ...
            <div class="form-item">
                <label>角色</label>
                <select name="roleIds" multiple style="min-height: 100px;">
                    ${roles.map(r => `<option value="${r.id}">${r.name}</option>`).join('')}
                </select>
                <div style="margin-top: 4px; font-size: 12px; color: #666;">
                    提示：按住Ctrl（Mac: Cmd）键可多选
                </div>
            </div>
            <div class="form-item">
    ...
    showModal('添加用户', content, async () => {
        ...
        const selectedRoleIds = Array.from(roleSelect.selectedOptions).map(opt => parseInt(opt.value));
        ...
        const content = `
            <form id="editUserForm">
                ...
                <div class="form-item">
                    <label>角色</label>
                    <select name="roleIds" multiple style="min-height: 100px;">
                        ${roles.map(r => `<option value="${r.id}" ${user.roleIds && user.roleIds.includes(r.id) ? 'selected' : ''}>${r.name}</option>`).join('')}
                    </select>
                    <div style="margin-top: 4px; font-size: 12px; color: #666;">
                        提示：按住Ctrl（Mac: Cmd）键可多选
                    </div>
...
async function showAddRoleModal() {
    ...
    const content = `
        <form id="addRoleForm">
            ...
            <div class="form-item">
                <label>权限</label>
                <select name="permissionIds" multiple style="min-height: 150px;">
                    ${permissions.map(p => `<option value="${p.id}">${p.name} (${p.code})</option>`).join('')}
                </select>
                <div style="margin-top: 4px; font-size: 12px; color: #666;">
                    提示：按住Ctrl（Mac: Cmd）键可多选
                </div>
    ...
    showModal('添加角色', content, async () => {
        ...
        const permissionSelect = form.querySelector('[name="permissionIds"]');
        const selectedPermissionIds = Array.from(permissionSelect.selectedOptions).map(opt => parseInt(opt.value));
        ...
        const content = `
            <form id="editRoleForm">
                ...
                <div class="form-item">
                    <label>权限</label>
                    <select name="permissionIds" multiple style="min-height: 150px;">
                        ${permissions.map(p => `<option value="${p.id}" ${role.permissionIds && role.permissionIds.includes(p.id) ? 'selected' : ''}>${p.name} (${p.code})</option>`).join('')}
                    </select>
                    <div style="margin-top: 4px; font-size: 12px; color: #666;">
                        提示：按住Ctrl（Mac: Cmd）键可多选
                    </div>
