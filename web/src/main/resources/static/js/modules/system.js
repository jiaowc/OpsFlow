...
function showConfigTab(tabName, element) {
    ...
    try {
        ...
        systemSection.querySelectorAll('.tab-buttons > .tab-btn').forEach(btn => btn.classList.remove('active'));
        systemSection.querySelectorAll('.config-panel').forEach(panel => panel.classList.remove('active'));
    ...
    switch(tabName) {
        ...
        case 'pipeline-mgmt':
            ...
            break;
        case 'template':
                const firstTemplateBtn = systemSection.querySelector('.template-tab-btn[data-template-type="dockerfile"]');
...
function showTemplateTab(templateType, element) {
    ...
    systemSection.querySelectorAll('.template-tab-btn').forEach(btn => btn.classList.remove('active'));
    systemSection.querySelectorAll('.template-sub-panel').forEach(panel => panel.classList.remove('active'));
...
function loadDeploymentTemplates() {
    const list = document.getElementById('deploymentTemplateList');
    if (!list) return;
    list.innerHTML = '<tr><td colspan="5" style="text-align: center; padding: 40px; color: #999;">暂无 Deployment 模版</td></tr>';
}
window.loadDeploymentTemplates = loadDeploymentTemplates;

function loadDockerfileTemplates() {
...
function renderPipelineBuildNodeMultiSelect(pipeline) {
    ...
    const displayHtml = selectedNodes.length
        ...
        : '<span class="tag-placeholder">请选择一个或多个构建节点</span>';
...
function syncPipelineBuildNodeSelection() {
    ...
    if (!checked.length) {
        display.innerHTML = '<span class="tag-placeholder">请选择一个或多个构建节点</span>';
        return;
    }
...
function buildPipelineNodeSectionHtml(pipeline) {
    ...
    return `
        <div style="border: 1px solid #bfdbfe; border-radius: 6px; padding: 16px; margin-bottom: 16px; background: #eff6ff;">
            <h4 style="margin: 0 0 12px 0; color: #1d4ed8;">CI 阶段（持续集成）</h4>
            <div class="form-item">
                <label>候选构建节点 *</label>
                <div id="pipelineBuildNodeMultiSelect" style="position:relative;">
                    ${renderPipelineBuildNodeMultiSelect(pipeline)}
                </div>
                <small style="color:#666;display:block;margin-top:4px;">可选择一个或多个构建节点；运行 Job 时会优先调度到当前负载较低的节点执行 CI 步骤</small>
            </div>
            <div class="form-item">
        ...
        <div style="border: 1px solid #bbf7d0; border-radius: 6px; padding: 16px; background: #f0fdf4;">
            <h4 style="margin: 0 0 12px 0; color: #15803d;">CD 阶段（持续部署）</h4>
            <div class="form-item">
                <label>部署节点 *</label>
                <select name="deployNodeId" required>${renderPipelineNodeSelectOptions('deploy', pipeline.deployNodeId)}</select>
                <small style="color:#666;display:block;margin-top:4px;">部署、检查部署状态等步骤在部署节点执行</small>
            </div>
            <div class="form-item">
                <label style="display:flex;justify-content:space-between;align-items:center;">
                    <span>CD 步骤 *</span>
...
function collectPipelineFormData(form) {
    ...
    const buildNodeIds = buildNodeInput && buildNodeInput.value
        ...
        : [];
    const deployNodeId = form.querySelector('[name="deployNodeId"]')?.value;
    if (!buildNodeIds.length) {
        alert('请至少选择一个 CI 构建节点');
        return null;
    }
    if (!deployNodeId) {
        alert('请选择 CD 部署节点');
    ...
    return {
        ...
        buildNodeId: buildNodeIds[0],
        buildNodeIds: buildNodeIds,
        deployNodeId: parseInt(deployNodeId, 10),
        steps: steps
    };
}
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (pipelines && pipelines.length > 0) {
                ...
                pipelineList.innerHTML = '<tr><td colspan="4" style="text-align: center; padding: 40px; color: #999;">暂无 Pipeline 模版</td></tr>';
            }
        } else {
            ...
            <div class="pipeline-flow-tracks">
                <div class="pipeline-flow-track pipeline-flow-ci">
                    <div class="pipeline-flow-track-header">
                        ...
                        <span class="pipeline-flow-node" title="CI 构建节点">${escPipelineHtml(buildNodeName || '-')}</span>
                    </div>
