...
function isNonProdBuildEnv(env) {
    if (!env) return false;
...
async function loadBuildJobs() {
    try {
        ...
        if (envsResponse.ok) {
            ...
            // 使用模拟数据
            envs = mockEnvs.filter(isNonProdBuildEnv);
        }
        envs = (envs || []).filter(isNonProdBuildEnv);
...
async function showAddBuildTaskModal() {
    ...
    try {
        ...
        if (pipelineResponse.ok) {
            pipelines = (await pipelineResponse.json() || []).filter(p =>
    ...
    if (pipelines.length === 0) {
        alert('暂无可用 CI/CI/CD 流水线模版，请先在「流水线配置 → 流水线管理」中创建模版');
        return;
    }
        ...
        <form id="addBuildTaskForm" style="max-height: 80vh; overflow-y: auto;">
            ...
            <div class="form-item">
                ...
                <select name="ENV" required>
                    ...
                    ${envs.filter(isNonProdBuildEnv).map(e => `<option value="${e.name}">${e.name}</option>`).join('')}
                </select>
            </div>
