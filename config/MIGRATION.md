...
### 项目内配置文件（开发环境）
- **位置**: `admin/src/main/resources/application.yml`
- **用途**: 开发环境默认配置
- **Git**: ✅ 提交到 Git（不包含敏感信息）
...
```bash
# 1. 复制配置文件
cp admin/src/main/resources/application.yml config/application.yml
...
### Q: 如果外部配置不存在会怎样？
A: 应用会使用项目内的默认配置（`admin/src/main/resources/application.yml`）。
